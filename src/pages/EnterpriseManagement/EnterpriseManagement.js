...
import { renderCensorTag, renderStatus, renderFunctions, formatTime, renderProhibitText, formatStringArray, renderBatchFunctions, formatTokensData, handleRes } from '@/utils/commonFunction';
// import { MockSelect } from '@/utils/Enum';
import styles from './EnterpriseManagement.less';

// const { categoryTokenSelect, stateSelect, censorSelect } = MockSelect;
const { TabPane } = Tabs;
const { confirm } = Modal;
...
class EnterpriseManagement extends Component {
    ...
    confirm({
      ...
      onOk() {
        dispatch({
          ...
          callback: (res) => {
            if (res) {
              handleRes({ res, actionName, record, showName: 'companyName', freshData, cleanRows, pagination });
            }
          }
        })
      },
  ...
  onOperation = (props) => {
    ...
    if (prefix) {
      this.onConfirm({ ...props, dispatch, freshData, pagination, cleanRows: () => this.handleSelectRows([]) })
    }
  }
  ...
  render() {
    ...
    const columns = [
      { title: '企业名称', key: 'companyName', render: record => <a onClick={() => this.handleDetail(record)}>{record.companyName}</a> },
