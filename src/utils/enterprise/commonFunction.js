...
import { CommonEnum, AllButtonConfig, BatchButtonConfig } from '../Enum';
...
const getSingleButtonConfig = props => {
  ...
  const { pickState, pickSale, pickCensor, pickProhibit, pickChange } = property;
...
const formatStringArray = (content) => <div>{content.map(i => i)}</div>
...
const formatSelectOption = (props) => {
  ...
  const result = [];
  array.forEach(i => result.push({ 'label': i[label], 'value': i[value], 'key': i[value] }))
  return result
}

const formatCategoryNames = (array) => array.map(i => i.replace(/\\/g, ''))
  ...
  if (initialFormData) {
    Object.keys(initialFormData).forEach(key => {
      switch (key) {
        ...
        case 'categoryNames':
        case 'images':
          result[key] = formatCategoryNames(initialFormData[key]);
          break;
        default:
...
const renderOmitProductLanguage = content => _.omit(content, productLanguageTemplate)
...
const renderOmitEnterpriseLanguage = content => _.omit(content, enterpriseLanguageTemplate)
...
const renderOmitCategoryLanguage = content => _.omit(content, categoryLanguageTemplate)
...
const handleRes = (props) => {
  const { res, actionName, record, showName, freshData, cleanRows, pagination } = props;
...
const renderProhibitInTable = (props) => {
  ...
  if (name === 'language') {
    return record && record.prohibit ?
      <Badge count={<Icon type="exclamation-circle" style={{ color: '#8c8c8c', paddingLeft: '20px' }} theme="filled" title="已被禁用,请联系平台客服人员" />}>
...
const renderProhibitByAlert = (prohibit) => prohibit ? <Alert message="已被禁用,请联系平台客服人员" type="warning" showIcon style={{ marginBottom: '5px' }} /> : null
