...
import { Form, Col, Row, Select, Input, Checkbox, InputNumber, DatePicker, Switch } from 'antd';
      ...
      <Form layout="inline" className={styles.tableListForm} style={style}>
        <Row gutter={gutter}>
          {modalFormConfig.map(item => {
            ...
            const colConfig = { span, offset, key: fieldId, style: { height: '56px' } };
            const formItemConfig = {
              label,
              labelCol: { span: labelSpan || (span === 8 ? 6 : span === 12 ? 4 : 2) },
            };
            const initV = initialFormData[fieldId] !== undefined ? initialFormData[fieldId] : initialValue;
            switch (fieldType) {
              case 'input':
                return (
                  <Col {...colConfig}>
